...
EOF
  ...
  # Modern dnf/Fedora may show upgrades differently, so we handle both paired and unpaired formats
  # Modern dnf uses "Replaced" instead of "Upgraded" for the old package version
  # Also, dnf may list the same package multiple times in the history
  LAST_UPGRADE=$(echo "$YUM_HISTORY" | grep "Upgraded \| Upgrade \| Updated \| Update \| Replaced " | grep -v -E '\*|\*\*|#|##')

  # Separate new and old package lines
  UPGRADE_NEW=$(echo "$LAST_UPGRADE" | grep " Upgrade \| Update ")
  UPGRADE_OLD=$(echo "$LAST_UPGRADE" | grep " Upgraded \| Updated \| Replaced ")

  # Remove duplicate package entries (dnf sometimes lists packages twice)
  UPGRADE_NEW=$(echo "$UPGRADE_NEW" | awk '!seen[$2]++')
  UPGRADE_OLD=$(echo "$UPGRADE_OLD" | awk '!seen[$2]++')

  # Count lines to determine if we have pairs or need to process differently
  NUM_NEW=$(echo "$UPGRADE_NEW" | grep -c "^")
  NUM_OLD=$(echo "$UPGRADE_OLD" | grep -c "^")
...
EOF
  ...
  # If we have matching pairs, process as pairs
  if [ "$NUM_NEW" -eq "$NUM_OLD" ] && [ "$NUM_NEW" -gt 0 ]; then
    while read -r line_update_new && read -r line_updated_old <&3; do
      if [ -z "$line_update_new" ]; then
        continue
      fi
      ### New (Upgrade/Update)
      ...
      echo -n "    }" | tee -a "${RESULT_FILE}"
      comma=','
    done < <(echo "$UPGRADE_NEW") 3< <(echo "$UPGRADE_OLD")
  else
    # If not paired (modern dnf format), just process Upgrade/Update lines
    while read -r line; do
      if [ -z "$line" ]; then
        continue
      fi
